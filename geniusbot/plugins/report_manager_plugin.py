...
class ReportManagerTab(QWidget):
    def __init__(self, console):
        ...
        self.custom_report_generate_label = QLabel(
        ...
        self.custom_data_file_label = QLabel(
            f'{os.path.expanduser("~")}'.replace("\\", "/")
        )
        self.report_file_location_button.setStyleSheet(
        ...
        self.generated_report_save_location_button.clicked.connect(
            self.report_manager_save_location
        )
        ...
        self.dataframe_label.setText("Dataframe will appear here\n")
        ...
        self.custom_report_layout.addWidget(
            self.generated_report_save_location_button, 0, 0, 1, 1
        )
        self.custom_report_layout.addWidget(
            self.custom_report_generate_label, 0, 1, 1, 5
        )
        self.custom_report_layout.addWidget(
            self.report_file_location_button, 1, 0, 1, 1
        )
        ...
        self.custom_report_layout.addWidget(
            self.custom_report_generate_button, 5, 0, 1, 6
        )

        self.merged_report_save_location_button = QPushButton("Save Location")
        self.merged_report_save_location_button.setStyleSheet(
            f"background-color: {orange}; color: white; font: bold;"
        )
        self.merged_report_save_location_button.clicked.connect(
            self.report_merger_save_location
        )
        self.merged_report_save_location_label = QLabel(
            f'{os.path.expanduser("~")}'.replace("\\", "/")
        )
        ...
        self.merge_file1_location_button.setStyleSheet(
            f"background-color: {green}; color: white; font: bold;"
        )
        self.merge_file2_location_button.setStyleSheet(
            f"background-color: {green}; color: white; font: bold;"
        )
        ...
        self.file1_columns.setSelectionMode(
            QAbstractItemView.SelectionMode.ExtendedSelection
        )
        self.file2_columns.setSelectionMode(
            QAbstractItemView.SelectionMode.ExtendedSelection
        )
        ...
        self.merge_report_layout.addWidget(
            self.merged_report_save_location_button, 2, 0, 1, 1
        )
        self.merge_report_layout.addWidget(
            self.merged_report_save_location_label, 2, 1, 1, 3
        )
    ...
    def open_report_manager_file(self):
        ...
        report_manager_data_file = QFileDialog.getOpenFileName(
            self, "Open data (CSV/XLSX)"
        )
    ...
    def open_data1_file(self):
        ...
        report_manager_data_file = QFileDialog.getOpenFileName(
            self, "Open data file 1(CSV/XLSX)"
        )
    ...
    def open_data2_file(self):
        ...
        report_manager_data_file = QFileDialog.getOpenFileName(
            self, "Open data file 2(CSV/XLSX)"
        )
    ...
    def report_manager_save_location(self):
        ...
        report_save_directory = QFileDialog.getExistingDirectory(
            None,
            "Select a folder:",
            os.path.expanduser("~"),
    ...
    def report_manage(self):
        ...
        self.report_manager_worker = ReportManagerWorker(
            self.report_manager,
            self.report_name_editor,
            self.custom_report_generate_label,
            self.action_type_combobox,
            self.pandas_profiling_ticker,
            self.custom_report_ticker,
        ...
        self.report_manager_worker.finished.connect(
            self.report_manager_worker.deleteLater
        )
        self.report_manager_thread.finished.connect(
            self.report_manager_thread.deleteLater
        )
    ...
    def merge_reports(self):
        ...
        self.merge_report_worker = MergeReportWorker(
            self.report_manager,
            self.file1_columns,
            self.file2_columns,
            self.action_type_combobox,
            self.merge_type_combobox,
            self.merged_report_save_location_label,
            self.merge_file1_label,
            self.merge_file2_label,
            self.merged_report_name_editor,
...
class ReportManagerWorker(QObject):
    ...
    def __init__(
        ...
        report_manager,
        report_name_editor,
        custom_report_generate_label,
        action_type_combobox,
        pandas_profiling_ticker,
        custom_report_ticker,
        file_type_combobox,
    ):
        super().__init__()
    ...
    def run(self):
        ...
        if self.action_type_combobox.currentText() == "Generate Report":
            if self.pandas_profiling_ticker.isChecked:
                ...
                minimal_flag = (
                    False  # Quicker run if set to true, but not everything is captured.
                )
                self.report_manager.create_pandas_profiling_report(
                    sample_flag, minimal_flag
                )
                self.report_manager.export_pandas_profiling()
            if self.custom_report_ticker.isChecked():
                self.report_manager.run_analysis()
                self.report_manager.export_data(
                    csv_flag=self.csv_flag,
...
class MergeReportWorker(QObject):
    ...
    def __init__(
        ...
        action_type_combobox,
        merge_type_combobox,
        merged_report_save_location_label,
        merge_file1_label,
        merge_file2_label,
        merged_report_name_editor,
    ...
    def run(self):
        ...
        self.report_manager.set_save_directory(
            self.merged_report_save_location_label.text()
        )
        if (
            self.action_type_combobox.currentText() == "Merge Reports"
            ...
            self.report_manager.load_dataframe(file_instance=2)
            self.report_manager.load_dataframe(file_instance=3)
            self.report_manager.set_join_type(
                join_type=self.merge_type_combobox.currentText().lower()
            )
