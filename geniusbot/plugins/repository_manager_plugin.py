...
if check_package(package="repository-manager"):
...
class RepositoryManagerTab(QWidget):
    def __init__(self, console):
        ...
        self.repository_manager_repositories_location_button = QPushButton(
        ...
        self.repository_manager_repositories_location_label = QLabel(
            f'{os.path.expanduser("~")}'.replace("\\", "/")
        )
        self.repository_manager_repositories_file_location_button = QPushButton(
            "Repositories File Location"
        )
        self.repository_manager_repositories_file_location_button.setStyleSheet(
            f"background-color: {green}; color: white; font: bold;"
        )
        self.repository_manager_repositories_file_location_button.clicked.connect(
            self.open_repository_manager_file
        )
        ...
        self.repository_git_command_label.setStyleSheet(
        ...
        self.repository_links_editor_label.setStyleSheet(
            "color: black; font-size: 11pt;"
        )
        ...
        self.repository_manager_files_label.setText("Repositories will be shown here\n")
        self.repository_manager_files_label.setFont("Arial")
        self.repository_manager_files_label.setFontColor(
        ...
        repository_manager_layout.addWidget(
            self.repository_manager_repositories_location_button, 0, 0, 1, 1
        )
        repository_manager_layout.addWidget(
            self.repository_manager_repositories_location_label, 0, 1, 1, 2
        )
        repository_manager_layout.addWidget(
            self.repository_manager_repositories_file_location_button, 1, 0, 1, 1
        )
        repository_manager_layout.addWidget(
            self.repository_manager_file_location_label, 1, 1, 1, 2
        )
        ...
        repository_manager_layout.addWidget(
            self.repository_git_command_label, 3, 0, 1, 3
        )
        repository_manager_layout.addWidget(self.repository_git_command, 4, 0, 1, 3)
        repository_manager_layout.addWidget(
            self.repository_links_editor_label, 5, 0, 1, 3
        )
        repository_manager_layout.addWidget(self.repository_links_editor, 6, 0, 1, 3)
        repository_manager_layout.addWidget(
            self.repository_manager_files_label, 7, 0, 1, 3
        )
        repository_manager_layout.addWidget(
            self.repository_manager_run_button, 8, 0, 1, 3
        )
    ...
    def manage_repositories(self):
        ...
        self.repository_manager_worker = RepositoryManagerWorker(
            self.repository_manager,
            self.clone_ticker,
            self.set_default_branch_ticker,
            self.pull_ticker,
        ...
        self.repository_manager_thread.started.connect(
            self.repository_manager_worker.run
        )
        self.repository_manager_worker.finished.connect(
            self.repository_manager_thread.quit
        )
        self.repository_manager_worker.finished.connect(
            self.repository_manager_worker.deleteLater
        )
        self.repository_manager_thread.finished.connect(
            self.repository_manager_thread.deleteLater
        )
        self.repository_manager_worker.progress.connect(
            self.report_repositories_progress_bar
        )
    ...
    def open_repository_manager_file(self, projects=None):
        ...
        repository_manager_file_location_name = QFileDialog.getOpenFileName(
            self, "Text File with Repositories"
        )
        if (
            repository_manager_file_location_name[0] is None
            or repository_manager_file_location_name[0] == ""
        ):
            repository_manager_file_location_name = os.path.expanduser("~")
        self.repository_manager_file_location_label.setText(
            repository_manager_file_location_name[0]
        )
    ...
    def repository_manager_repositories_location(self):
        ...
        repository_manager_location_name = QFileDialog.getExistingDirectory(
            None,
            "Select a folder:",
            os.path.expanduser("~"),
        ...
        if (
            repository_manager_location_name is None
            or repository_manager_location_name == ""
        ):
            repository_manager_location_name = os.path.expanduser("~")
        self.repository_manager_repositories_location_label.setText(
            repository_manager_location_name
        )
        self.repository_manager.set_repository_directory(
            repository_manager_location_name
        )
        repositories_string = ""
...
class RepositoryManagerWorker(QObject):
    ...
    def __init__(
        ...
        repository_manager,
        clone_ticker,
        set_default_branch_ticker,
        pull_ticker,
        repository_links_editor,
        repository_git_command,
        repository_manager_file_location_label,
        repository_manager_files_label,
        repository_manager_repositories_location_label,
    ):
        super().__init__()
    ...
    def run(self):
        ...
        if self.clone_ticker.isChecked():
            ...
            if os.path.exists(self.repository_manager_file_location_label.text()):
                try:
                    file_repositories = open(
                        self.repository_manager_file_location_label.text(), "r"
                    )
                    for repository in file_repositories:
                        projects.append(repository)
                    projects = list(dict.fromkeys(projects))
        ...
        if self.pull_ticker.isChecked():
            self.repository_manager.pull_projects(
                set_to_default_branch=default_branch_flag
            )
        ...
        if self.repository_git_command.text() != "":
            ...
            for project in projects:
                directory = os.path.join(
                    self.repository_manager_repositories_location_label.text(), project
                )
                try:
                    result = self.repository_manager.git_action(
                        command=f"{self.repository_git_command.text()}",
                        directory=f"{directory}",
                    )
                    print(result)
                except Exception as e:
                    print(
                        f"Unable to execute git command: {self.repository_git_command.text()} "
