...
sys.path.append("..")
try:
    from qt.colors import green, orange, blue
...
class MediaManagerTab(QWidget):
    def __init__(self, console):
        ...
        self.media_manager_media_location_button.setStyleSheet(
            f"background-color: {orange}; color: white; font: bold;"
        )
        self.media_manager_media_location_button.clicked.connect(
            self.media_manager_media_location
        )
        self.media_manager_media_location_label = QLabel(
            f'{os.path.expanduser("~")}'.replace("\\", "/")
        )
        self.media_manager_move_location_button = QPushButton("Move Location")
        self.media_manager_move_location_button.setStyleSheet(
            f"background-color: {green}; color: white; font: bold;"
        )
        self.media_manager_move_location_button.clicked.connect(
            self.media_manager_move_location
        )
        self.media_manager_move_location_label = QLabel(
            f'{os.path.expanduser("~")}'.replace("\\", "/")
        )
        ...
        self.media_manager_files_label.setText("Media files found will be shown here\n")
        self.media_manager_files_label.setFont("Arial")
        self.media_manager_files_label.setFontColor(
        ...
        media_manager_layout.addWidget(
            self.media_manager_media_location_button, 0, 0, 1, 1
        )
        media_manager_layout.addWidget(
            self.media_manager_media_location_label, 0, 1, 1, 1
        )
        media_manager_layout.addWidget(
            self.media_manager_move_location_button, 1, 0, 1, 1
        )
        media_manager_layout.addWidget(
            self.media_manager_move_location_label, 1, 1, 1, 1
        )
    ...
    def manage_media(self):
        ...
        self.media_manager_worker = MediaManagerWorker(
        ...
        self.media_manager_worker.finished.connect(
            self.media_manager_worker.deleteLater
        )
        self.media_manager_thread.finished.connect(
            self.media_manager_thread.deleteLater
        )
    ...
    def media_manager_media_location(self):
        ...
        media_manager_directory_name = QFileDialog.getExistingDirectory(
            None,
            "Select a folder:",
            os.path.expanduser("~"),
    ...
    def media_manager_refresh_list(self):
        self.media_manager.set_media_directory(
            self.media_manager_move_location_label.text()
        )
        self.media_manager.find_media()
        files = ""
        for file in self.media_manager.get_media_list():
    ...
    def media_manager_move_location(self):
        ...
        media_manager_move_directory_name = QFileDialog.getExistingDirectory(
            None,
            "Select a folder:",
            os.path.expanduser("~"),
            QFileDialog.Option.ShowDirsOnly,
        ...
        if (
            media_manager_move_directory_name is None
            or media_manager_move_directory_name == ""
        ):
            media_manager_move_directory_name = os.path.expanduser("~")
        self.media_manager_move_location_label.setText(
            media_manager_move_directory_name
        )
